...
(defn read-grid
  ...
  (let [grid (->> s split-lines vec (mapv vec))
    ...
    (if (every? #(= (count %) first-len) grid)
      (with-meta grid {:type ::grid
                       :bounds [(count grid) first-len]
...
(defn ->map
  "Convert a grid to a priority map, or keeps a priority map as it is.
  ...
  (case (type g)
    ...
    ::grid (-<>>
            ...
            (into {})
            (with-meta <> (assoc (meta g)
                                 :type ::map)))

    (throw (Exception. "Requires a grid or priority map of grid."))))

(defn ->grid
  "Convert a p-map to a grid, or keep a grid as it is."
  [m]
  (case (type m)
    ::grid m

    ::map (let [[rb cb] (bounds m)]
            (-<>> (repeat cb \#) vec
                  (repeat rb) vec
                  (with-meta <> (meta m))
                  (reduce (fn [acc [k v]]
...
(defn ->string
  "Return a string representation of a grid or map.
  Optionally accepts a path."
  ([g]
   (->string g ()))
  ([g path]
   (-<>> (->grid g)
